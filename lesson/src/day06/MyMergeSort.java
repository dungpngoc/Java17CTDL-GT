package day06;

import java.util.Arrays;

public class MyMergeSort {
    public static void main(String[] args) {
//        int[] a1 = {1, 3, 5, 7};
//        int[] a2 = {2, 4, 6, 8};
//        System.out.println(Arrays.toString(merge2Array(a1, a2)));

        int[] arr = {1, 5, 3, 2, 8, 7, 6, 4};
        System.out.println("array: " + Arrays.toString(arr));
        int[] res = mergeSort(arr,0,arr.length - 1);
        System.out.println("array: " + Arrays.toString(res));
    }

    private static int[] merge2Array(int[] a1, int[] a2) {
        // a1, a2 sorted
        int[] res = new int[a1.length + a2.length];
        int idx = 0; // current index for updating
        int i = 0; // for a1
        int j = 0; // for a2
        while (i < a1.length && j < a2.length) {
            // break khi i hoặc j chạy hết mảng
            if (a1[i] < a2[j]) {
                // phần tử đang xét tại a1 < phần tử đang xét tại a2
                res[idx] = a1[i]; // thêm a[i] vảo res -> a[i] xem như đã được remove
                i++; // tăng i để xét phần tử tiếp theo
            } else {
                res[idx] = a2[j];
                j++;
            }
            idx++;
        }
        while (i < a1.length) { // a1 còn, chắc chắn a2 đã hết
            // điền tất cả phần tử còn lại của a1 vào res
            res[idx] = a1[i];
            idx++;
            i++;
        }
        while (j < a2.length) {
            res[idx] = a2[j];
            idx++;
            j++;
        }
        // return res[] được sắp xếp, merge từ a1 và a2
        return res;
    }
    private static int[] mergeSort(int[] arr, int left, int right) {
        if(left == right) {
            return new int[]{arr[left]};
        }
        int mid = left + (right - left) / 2;
        int[] a1 = mergeSort(arr,left,mid);
        int[] a2 = mergeSort(arr,mid + 1, right);
        int[] res = merge2Array(a1,a2);
        return res;
    }

//    private static int[] mergeSort(int[] arr, int left, int right) {
//        // Mảng có duy nhất 1 phần tử -> chắc chắn sorted
//        // Lấy ra mảng có 1 phần tử
//        System.out.println("Chia đôi phần tử " + left + " đến " + right);
//        if (left == right) { // base condition
//            System.out.println("Mảng có 1 phần tử index = " + left + " ,value = " + arr[left]);
//            return new int[]{arr[left]};
//        }
//        int mid = left + (right - left) / 2;
//        int[] a1 = mergeSort(arr, left, right);
//        int[] a2 = mergeSort(arr, mid + 1, right);
//        int[] res = merge2Array(a1, a2);
//        System.out.println("Done merge từ " + left + " đến " + right);
//        System.out.println(Arrays.toString(arr));
//        return res;
//    }
}
